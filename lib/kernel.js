...
// The interactive notebook must use the same Python environment that has the
// ML packages installed. A server started from Finder or another shell can
// have a different PATH from the terminal, so resolving only `python3` may
// silently select /usr/bin/python3 without torch. Allow an explicit override,
// then prefer the local ML environment, and finally fall back to PATH.
function pythonExecutable() {
  const candidates = [
    process.env.PYTHON_BIN,
    process.env.VIRTUAL_ENV ? path.join(process.env.VIRTUAL_ENV, 'bin', 'python3') : null,
    '/Users/air/env_th/bin/python3',
    'python3',
  ].filter(Boolean);
...
function startKernel(name) {
  ...
  const proc = spawn(pythonExecutable(), ['-u', workerPath], { stdio: ['pipe', 'pipe', 'pipe'] });
