...
function inline(rawText, existingPages) {
  ...
  out = out.replace(/\[([^\]\n]+)\]\(([^)\n]+)\)/g, (_, label, url) => {
    ...
    // Audio/video files get an inline player - left-click plays, right-click
    // still offers "Save As" from the browser's native context menu.
...
function buildTree(rootName) {
  ...
  return { root, nodes: pages.map((id) => ({ id })), edges };
}
