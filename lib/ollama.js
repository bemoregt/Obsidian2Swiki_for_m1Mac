...
// Translates a short piece of text (the video description pulled out of a
// page) into English, steering clear of this wiki's own tag characters
// (*, !) so the result can't accidentally turn into a bogus link or
// emphasis run once it's appended back into the page.
async function translateToEnglish(pageTitle, text) {
  const prompt =
    `다음은 위키 문서 "${pageTitle}"에 첨부된 동영상의 한국어 설명글이야. 이 설명을 자연스러운 영어로 번역해줘. ` +
    '조건: ' +
