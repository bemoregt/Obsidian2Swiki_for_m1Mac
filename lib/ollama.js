...
// skeleton), and returns their Python code as separate strings.
async function generateCoreFunctions(pageTitle, pageBody, count = 3) {
  const prompt =
    ...
    '4) 각 함수를 별도의 마크다운 코드블록(```python ... ```)으로, 코드 앞뒤에 다른 설명 문장 없이 코드만 답해.\n\n' +
    `--- 문서 내용 ---\n${pageBody}`;
  ...
  const blocks = [...text.matchAll(/```(?:python)?\s*\n([\s\S]*?)```/gi)]
    .map((m) => m[1].trim())
    .filter(Boolean);
  const codes = (blocks.length ? blocks : [text]).slice(0, count);
  if (!codes.length) throw new Error('핵심 함수 코드를 생성하지 못했습니다.');
  return codes;
}
