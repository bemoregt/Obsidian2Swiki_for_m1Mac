...
- **키워드로 RSS Feed 찾기**: 보기 화면 하단 📡 버튼 — 문서 제목을 키워드로 Google 뉴스·Bing 뉴스·Reddit의 실제 작동하는 RSS 검색 주소 3개를 문서 맨 아래에 추가함. 채용정보 찾기와 같은 이유로 AI가 피드 주소를 지어내지 않고, 키워드로 직접 검색되는 진짜 RSS 엔드포인트만 나열함. 각 링크를 클릭하면 원본 XML이 아니라, 서버가 `rss-parser`로 피드를 직접 읽어와 기사 제목·날짜·요약을 보기 좋은 목록으로 렌더링한 페이지(`/rss-view`)가 열리고, 그 위에 원본 RSS 주소도 함께 표시되어 피드 리더 구독용으로 복사할 수 있음
