...
- **3D 지식 그래프**: 사이드바 🕸️ 지식 그래프 — 위키의 `*링크*` 연결을 트리 구조로 단순화해 Three.js(WebGL)로 그린 3D 네비게이터 (영화 쥐라기공원의 유닉스 파일 네비게이터 느낌). `index` 페이지(없으면 가장 많이 연결된 페이지)를 가까이 두고, 링크가 깊어질수록 멀리·작게 배치해 원근감을 줌. 각 페이지는 하늘색 납작한 직육면체로, 부모-자식 연결은 바닥에 깔린 카키색 띠로 표시되고 그림자가 드리워져 입체적으로 보임. 바닥은 어두운 분홍색 불투명 단색 평면(격자선 없음), 배경은 지평선 부근이 밝은 초록으로 빛나고 위·아래로 갈수록 검정으로 어두워지는 그러데이션. 드래그로 회전, 스크롤로 확대/축소, 우클릭 드래그로 이동, 박스나 그 박스로 이어지는 링크 띠를 클릭하면(링크를 클릭하면 클릭한 지점에서 더 가까운 쪽 끝 노드가 선택됨 — 하위쪽뿐 아니라 상위쪽으로도 이동 가능) 연노란색 스포트라이트(경계가 또렷하고 훨씬 좁게 좁힌 원형 조명 + 반투명한 빛줄기가 각광처럼 그 지점만 비춤)가 켜지며 그 노드에서 뻗어나가는 링크 방향을 낮은 고도에서 가까이 바라보도록 카메라가 천천히 줌인하고, 패널에서 해당 페이지로 이동하는 버튼이 뜸 (three.js는 로컬에 내장되어 오프라인 동작)
